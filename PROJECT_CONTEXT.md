...
- ⚡ **EXTRA TURN / 1 MORE! 아케이드 컷인 (`ExtraTurnCutin.tsx`)**:
  ...
  - 하단 `[Space]` 공격 버튼에 골든 썬더 펄스 글로우를 부여하여 연속 공격 가능 상태를 직관적으로 강조.
- 💥 **스킬별 폭발 이펙트 & 오버킬 연쇄 관통 파티클 (`CombatFxLayer.tsx`)**:
  - 방패 강타 충격파 링, 광전사의 진노 폭주 화염, 전장의 함성 전장 충격파, 휠윈드 5레인 폭풍 회오리 VFX 고도화.
  - 전열에서 후열로 번개가 파파박 튀는 오버킬 연쇄 파열 스파크 라인.
- 🔒 **Zero Layout Shift 완벽 보장**: 모든 연출 레이어가 `absolute pointer-events-none` GPU 가속으로 작동하여 5레인 격자 및 하단 독 크기가 1px도 흔들리지 않음.
