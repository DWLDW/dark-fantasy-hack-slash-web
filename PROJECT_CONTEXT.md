...
### 27. 전수 시스템 버그 리뷰 & 경제/전투/진행도 전면 정상화 완료
- ⚔️ **전투 엔진 & 보스 AI 12대 핵심 버그 수정**:
  - **반격 턴 흡혈/보호막 보존**: 공격 턴에서 회복된 체력/보호막이 반격 턴 클로저 캡처로 인해 이전 HP로 덮어써지던 버그를 최신 상태 명시 전달로 완전 해결.
  - **보스 차징/인텐트 불변 갱신**: 몬스터 턴 종료 시 `chargedSurvivors`로 덮어써 보스 멸망기 차징/스태거 체력/인텐트가 날아가던 문제를 불변 매핑으로 해결.
  - **보스 예고 공격(Telegraph) 부정 연산자(`!`) 역전 수정**: 안전 레인 피격 / 위험 레인 면역 버그를 정상 판정으로 수정.
  - **보스 그로기 & 결계(70% 뎀감) 영구 지속 해제**: 턴 경과 후 정상 해제되도록 조치.
  - **보스 스킬 피해 방어/저항/보호막 적용**: 트루 데미지 100% 관통 버그 수정 (`applyPlayerDefenses` 적용).
  - **광전사의 진노(Berserk) 자해 HP 10% 소모 추가 & 3연타 체력 누적 오류 수정**.
  - **선제 기습(Ambush) 전멸 시 룸 클리어/출구 연결 보장**.
- 💎 **소켓팅 & 경제 & 호라드릭 큐브 5대 버그 수정**:
  - **단일 룬 소켓 각인(`socketRuneIntoItem`) 100% 정상화**: `runesVault`에서 룬을 정상 차감하고 장비의 `stats` 및 `socketedRunes`에 실시간 각인.
  - **기드 도박 유니크/세트 고유 옵션 보존**: `realUniqueName`/`setName`이 누락되어 케인 감정 시 일반 매직템으로 다운그레이드되던 버그 완전 해결.
  - **호라드릭 큐브 장비 3개 승급 및 소켓 개방 레시피 정상화, 잠금 아이템 보호**.
  - **도박장 가방 60개 만충 시 슬롯 오버플로우 방지**.
- 🧬 **스탯 계산기 & 패시브/세트아이템 전수 연동**:
  - 소켓 장착 일반 룬 스탯 200% 중복 합산(Double Dipping) 제거.
  - 세트 아이템 보너스 스탯(`int`, `wis`, `cha`, `minDmg`, `maxDmg`, `critDamage`, `fortune`, `goldFind` 등) 전수 합산.
  - 패시브 스킬 3종(`elemental_attunement` 속성 룬 피해 증폭, `titan_juggernaut` 멸망기 저지력 증폭, `berserker_rage` 타격 분노 가산) 전투 연동.
- 🗺️ **던전 승리/진행도 & 스냅샷 누수 수정**:
  - 던전 승리 A랭크/난이도+2 보상 덮어쓰기 오타(`else` 누락) 수정.
  - 던전 1개 통째 건너뛰기(Skip) 및 대균열 티어 이중 증가 버그 완전 해결.
  - 승리 모달 닫기 시 과거 던전 스냅샷 누수(`setDungeonSnapshot(null)`) 해결.
- 🎨 **UI 모달 & z-index 최적화**:
  - `TownView.tsx` `selectedBaseItemId` ID 기반 실시간 파생(`useMemo`) 전환 및 방패 스피리트 호환.
  - `CharacterModal.tsx` 스탯 초기화 샤드 수식 10배 오차(50~1000개 ➔ 5~100개) 수정.
  - 승리/사망 모달 `z-[120]` 상향으로 일반 모달과의 겹침 충돌 방지.
  - Base64 세이브 Export/Import 시 보관함(Stash) 및 패시브 레벨 유실 방지.

### 28. 유니크 / 룬워드 / 소켓 룬 미적용 특수 효과 & 메커니즘 100% 전수 구현 완료
- 👑 **유니크/세트 아이템 24종 허위 스탯 전수 인게임 구현 (`items.ts`, `types/game.ts`, `statCalculator.ts`, `combatEngine.ts`, `combatActionHelper.ts`)**:
  - **강타 (Crushing Blow, 10~35%)**: 기욤의 얼굴(Guillaume +35%), 고어 라이더(Gore Rider +15%) 장착 시 적중 시 몬스터 체력 25%(보스 10%)를 즉시 추가 파괴.
  - **악마 대상 추가 피해 (Damage to Demons, +350%)**: 안수(고무장갑, Laying of Hands) 장착 시 모든 엘리트/보스/악마 계열 몬스터에게 주는 최종 피해가 4.5배(+350%)로 폭증.
  - **타격 시 앰플리파이 (Amplify Damage 5%)**: 아트마 목걸이(Atma) 장착 시 5% 확률로 피격 몬스터의 방어력을 100% 분쇄(Armor Shred).
  - **타격 시 라이프 탭 (Life Tap 5%)**: 드라큘 장갑(Dracul's Grasp) 장착 시 5% 확률로 발동하여 흡혈율을 50%로 대폭 상향하고 흡혈 캡을 35%로 확장.
  - **결빙 방지 (Cannot be Frozen)**: 레이븐링(Raven Frost), 티리엘의 갑주(Tyrael's Might), 각운(Rhyme), 참 룬(Cham) 장착 시 서리/냉기/빙결 디버프 완전 면역.
  - **밀쳐내기 (Knockback)**: 바람살(Windforce) 공격 시 피격 생존 몬스터를 후열(`depth += 1`)로 시원하게 밀쳐냄.
  - **레벨 비례 스케일링**: 할배검(Grandfather, 레벨당 Min/Max Dmg +2.5), 샤코(Harlequin Crest, 레벨당 HP +1.5, 올스탯 +2), 불카토스(Bul-Kathos, 레벨당 HP +0.5).
  - **시곤 방패(Sigon's Guard)**: `+1 All Skills` 정식 추가.
- 🔮 **33종 전 룬워드 시스템 & 고유 오라/프로크/Off-Weapon ED 엔진 구축 (`runeWords.ts`, `statCalculator.ts`, `combatEngine.ts`)**:
  - **Off-Weapon 증뎀(ED) 곱연산 엔진**: 갑옷(인내 +200%), 방패(불사조 +150%)의 증뎀이 기본 데미지 0으로 증발하던 버그를 해결하고 `(1 + totalOffWeaponEd / 100)` 최종 데미지 곱연산 엔진 구축.
  - **무한(Infinity) 선고(Conviction) 오라**: 적 유효 방어력을 85% 즉시 파괴(`defense * 0.15`).
  - **불사조(Phoenix) 구원(Redemption) 오라**: 몬스터 처치(Kill) 시 플레이어의 체력 50% 및 분노 100% 즉시 완충.
  - **인내(Fortitude) 칠흑 갑주(Chilling Armor)**: 피격 시 공격한 몬스터에게 50% 결빙 반사 및 플레이어 방어력 +50% 버프.
  - **초승달(Crescent Moon) 스태틱 필드(Static Field)**: 10% 확률로 화면 내 살아있는 모든 몬스터의 현재 체력 25% 일괄 감축.
  - **슬픔/신념/자 룬 목표물 방어 무시(ITD)**: 적 방어력을 0으로 계산하여 100% 관통 피해.
  - **미등록 12종 룬워드 전수 완비**: 콜투암스(CTA), 오크의 심장(HOTO), 신념(Faith), 망명(Exile), 죽음(Death), 파멸(Doom), 자존심(Pride), 혼돈(Chaos), 용(Dragon), 정의의 손길(Hand of Justice), 서약(Oath), 추억(Memory).
- 💎 **33종 D2 룬(El ~ Zod) 스탯 불일치 및 소켓 집계 100% 일치화 (`runes.ts`, `statCalculator.ts`)**:
  - Ber(강타 20%), Jah(방어 무시), Cham(결빙 방지), Sur(적중 시 실명), Gul(명중률 +20%), Vex/Lem/Fal/Ko/Pul/Um/Mal/Ist 등 33종 전 룬의 툴팁과 실제 적용 스탯 100% 동기화.
  - 룬 소켓 시 누락되던 `damageReduction`, `goldFind`, `allSkills`, `turnRageRegen`, `rageCostReduction`, `evasion`이 종합 스탯에 100% 정상 집계.

### 30. [2단계 긴급 개편] 모바일 100dvh 제로 스크롤(Zero-Scroll) & 싱글 포커스 Diff 비교 전면 개편 완료
- 📱 **모바일 제로 스크롤 (100dvh No-Scroll) 아키텍처 수립**:
  - `App.tsx`의 `<main>`을 `flex-1 min-h-0 overflow-hidden`으로 고정하여 화면 전체의 불필요한 세로 스크롤 및 드래그 밀림을 100% 원천 차단.
  - `BottomDock.tsx`를 `relative flex-shrink-0`으로 Flex Flow에 안전하게 통합.
  - `BattleView.tsx`와 `TownView.tsx`의 고정 높이/비율을 제거하고 `h-full flex-1 min-h-0`으로 부모 뷰포트에 100% 자동 가변 핏.
- ⚔️ **싱글 포커스 Diff 카드 비교 시스템 (`ItemDetailCard.tsx`, `InventoryModal.tsx`)**:
  - 2개 카드를 무식하게 전체 복붙 나열하여 세로 스크롤이 생기던 구조를 전면 폐기.
  - **단일 후보 장비 카드 1개 중심의 싱글 포커스 Diff 카드**:
    - 헤더에 `vs [착용: 강철 검]` 명시.
    - 기본 공격력/방어력 영역에 `▲ +18` / `▼ -5` 인라인 증감 뱃지 통합.
    - 마법 속성(Affixes)을 2열 컴팩트 그리드로 압축하고 각 옵션 옆에 `(+5% ▲)` 델타 뱃지 인라인 표시.
    - 카드 하단에 **`[⚔️ 이 장비로 교체하기]`** 원터치 버튼을 직접 내장하여 [슬롯 선택 ➔ 후보 터치 ➔ 교체]가 세로 스크롤 없이 1화면에서 원터치 완료.
  - 인벤토리 모달 외곽을 `h-[96dvh] max-h-[96dvh] overflow-hidden`으로 고정.
- ⚡ **스킬 & 룬 각인 모달 0-스크롤 압축 (`SkillRuneModal.tsx`)**:
  - 상단 QWER 슬롯(44px) + 좌측 스킬 리스트(5cols) + 우측 3단 통합 패널(7cols: 정보 + 성장 + 5대 원소 룬 1열 소켓 바).
  - 모달 컨테이너 `h-[96dvh] overflow-hidden`으로 650px 모바일 화면에서도 0-스크롤 완결.
- 📊 **캐릭터 스탯 모달 500px 이하 초슬림 그리드 (`CharacterModal.tsx`)**:
  - 헤더(32px) + 좌측 6대 스탯 초슬림 1줄 행(24px) + 우측 4행 전투 테이블/1줄 저항 게이지로 500px 이하 원스크린 핏.
- 👑 **보스 HUD 64px 가로 초압축 HUD 전면 재설계 (`BossHUD.tsx`)**:
  - 기존 240px 거대 높이로 인해 전장을 아래로 밀어내던 BossHUD를 좌측 아바타(40px) + 중앙 HP/Break바 + 우측 약점 레인 버튼의 **64px 가로 인라인 HUD**로 전면 압축. 보스방에서도 5개 레인 전장이 절대 밀리지 않음.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).

### 31. [2단계 완료] 게임 진행 플로우(월드맵·승리·사망) 전면 개편 & 최고급 AI 일러스트 연출 연동
- 🗺️ **성역 대륙 월드맵 전면 개편 (`DungeonSelectView.tsx`)**:
  - **AI 고화질 월드맵 배경 탑재**: `public/images/ui/sanctuary_world_map.webp`를 배경으로 은은하게 적용하여 몰입감 극대화.
  - **100dvh 제로 스크롤 레이아웃**: 상단 타이틀 바(36px) + 1~5막/대균열 6버튼 탭(32px) + 좌측 5cols 던전 노드 + 우측 7cols 던전 정보/난이도 칩바/즉시 출격 버튼으로 모바일/PC 1화면 완결.
  - **키보드 단축키 지원**: `[1~5]` 액트 전환, `[Space/Enter]` 즉시 출격, `[Esc]` 마을 귀환.
- 🏆 **던전 승리 정산 아케이드 세레머니 (`DungeonVictoryModal.tsx`)**:
  - **AI 최고급 승리 일러스트 배너 탑재**: `public/images/ui/dungeon_victory_heroic.webp` (성당에서 악마를 처단하고 황금빛 서광과 전리품을 든 다크 판타지 유화).
  - **3대 재화 1줄 요약 바**: 골드 / 샤드 / 경험치 보상 한눈에 확인.
  - **전리품 뱃지 그리드**: 미확인 장비는 붉은 물음표 `?` 뱃지로 표시되며, 우측 상단 `[케인 감정 [F]]` 버튼으로 원터치 일괄 감정 지원.
  - **하단 고정 액션**: `[⚔️ 다음 진격 [Space]]` 및 `[마을 귀환 [Esc]]`로 손목 피로도 제로화.
- 💀 **심연의 전사/사망 모달 전면 개편 (`DeathModal.tsx`)**:
  - **AI 최고급 사망 일러스트 배너 탑재**: `public/images/ui/death_abyssal_defeat.webp` (붉은 달 아래 꽂힌 부러진 대검과 핏빛 안개).
  - **사망 원인 및 보존 항목 1줄 브리핑**: 미확정 전리품 소실 및 기존 장비/레벨 안전 안내.
  - **하단 원터치 액션**: `[⚔️ 즉시 재도전 [Space]]` 및 `[마을 귀환 [Esc]]`.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).

### 32. 인벤토리 UX·기드 도박 문구 품격화·룬워드 소켓 정렬/필터 전면 개선 완료
- 🎒 **인벤토리 교체 후보 가시성 & 수치 범위 표기 (`InventoryModal.tsx`)**:
  - 교체 후보 그리드 높이를 `max-h-[118px]`로 확장하여 최소 2~3줄이 시원하고 여유 있게 표시.
  - 무기는 `⚔️ minDmg~maxDmg` (예: `⚔️ 10~25`), 방어구는 `🛡️ defense`로 직관적인 전체 스탯 범위 출력.
- 📐 **인벤토리 헤더 레이아웃 재배치 (`InventoryModal.tsx`)**:
  - 가장 표준적인 UI 감각에 맞춰 닫기(`[X]`) 버튼을 헤더 맨 왼쪽으로 이동.
  - 우측 액션 도크에 `[💰 일반/마법 일괄판매]` (판매 가능 수량 뱃지 포함)와 `[⚡ 추천 일괄 장착 [A]]` 버튼을 나란히 배치.
- 🎲 **기드(Gamble) 암시장 도박 텍스트 다크 판타지 품격화 (`TownView.tsx`)**:
  - 특정 아이템 줄임말 나열(`조던/나겔`, `마라/대군주`, `샤코/크라운`)을 전면 제거.
  - `미지의 무기 (고대 병기 & 도검)`, `미지의 갑옷 (판금갑 & 성역 방호구)`, `미지의 반지 (신비로운 마력의 고리)`, `미지의 목걸이 (고대 유물 아뮬렛)` 등 정통 다크 판타지 명칭과 품격 있는 대사로 전면 개편.
- 🔮 **룬워드 공방 노멀 소켓 베이스 아이템 전용 필터 & 최고 품질순 정렬 (`TownView.tsx`)**:
  - **엄격한 노멀 소켓 전용 필터링 (`rarity === 'normal'`)**: 디아블로2 정통 룰에 따라 매직/레어/유니크 소켓 아이템을 공방 룬워드 베이스 목록에서 완전히 배제하고, 순수 **노멀(회색) 소켓 장비**만 목록에 노출.
  - **부위별 5대 필터 탭**: `[전체] [⚔️ 무기] [🥋 갑옷] [👑 투구] [🛡️ 방패]`와 각 부위별 노멀 소켓 보유 수량 뱃지 제공.
  - **다차원 복합 정렬 셀렉터**:
    - `💎 최고 베이스순 (기본)`: 소켓 수(6>1) ➔ 티어(Elite>Exceptional>Normal) ➔ 기본 스탯 순 자동 정렬.
    - `🔢 소켓 수 많은 순` / `⭐ 엘리트 티어순` / `⚔️ 공격/방어 스탯순`.
    - 안내 문구 명시: *"※ 매직/레어/유니크 소켓 장비는 룬워드 합성이 불가하며, 인벤토리에서 개별 룬 소켓팅만 가능합니다."*
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).
...
### 35. 닫기 버튼 우측 상단 일괄 통일 & 액트별 초선명 전장 아트 & 5대 보스 대형 와이드 포트레이트 배너 HUD 구축
- 🔲 **닫기(`[X]`) 버튼 전 모달 우측 상단(Top-Right) 일괄 통일 (`InventoryModal.tsx`, `SkillRuneModal.tsx`, `CharacterModal.tsx` 등)**:
  - 인벤토리 모달 헤더 구조를 `[좌측: 4개 모드 탭] ---------- [우측: 일괄판매 + 추천장착 + [X 닫기]]`로 표준화하여 전 모달 닫기 위치 완벽 통일.
- 🎨 **액트 1~5 초선명 시그니처 전장 배경 & 앰비언트 글로우 강화 (`BattleView.tsx`)**:
  - 액트별 랜드마크(1막 핏빛 안개 대성당, 2막 황금빛 모래궁전, 3막 비취빛 정글 지구라트, 4막 지옥 용암 오망성, 5막 코발트 설산 월드스톤)의 고유 색채를 극대화.
  - 배경 투명도를 `opacity-40`으로 대폭 강화하고 액트별 테두리 및 앰비언트 글로우(`actAmbientGlow`)를 결합하여 한눈에 액트가 확연히 구분되도록 업그레이드.
- 👹 **보스 HUD 110px~125px 대형 배너화 & 96px 대형 포트레이트 카드 전면 개편 (`BossHUD.tsx`)**:
  - `boss_andariel.webp`, `boss_duriel.webp`, `boss_mephisto.webp`, `boss_diablo.webp`, `boss_baal.webp` 연동.
  - 보스방 진입 시 상단에 **96px 거대 보스 포트레이트 카드**와 **`opacity-65` 고화질 와이드 보스 일러스트 배너**, **20px 2단 심연 체력바**, **전멸기 BREAK 충전 게이지**가 웅장하게 펼쳐지도록 전면 개편.
  - `하이 카운실 이스마일`, `트라빈칼 평의회`, `카운테스`, `스미스`, `라다먼트`, `소환술사`, `이자주얼`, `헤파스토`, `고대인` 등 액트별 네임드/중간보스 전수 매핑 완비.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).

### 36. 화면 절반을 장악하는 거대 보스 시네마틱 스테이지(Half-Screen Boss Arena) 전면 구축
- 👹 **마스킹 없는 100% 온전한 거대 보스 전신/반신 시네마틱 아레나 (`BossHUD.tsx`)**:
  - `max-height: 185px` 제한과 `object-cover` 잘림 현상을 전면 해결.
  - 상단 높이를 `clamp(220px, 36vh, 310px)` (화면의 약 40%)로 대폭 확장하고, **`object-contain` 렌더링을 적용하여 메피스토/디아블로/바알의 거대한 뿔, 해골 얼굴, 전신 아우라가 100% 온전하고 압도적인 크기로 화면 중앙에 군림**.
  - 보스 머리 위 `[👑 대악마 타이틀]` + `[속성 뱃지]`, 발밑 와이드 **2단 심연 체력 게이지 (20px)**, **전멸기 BREAK 카운터**, **약점 레인 조준 버튼** 통합.
  - 플레이어 공격 시 상단 거대 보스 아트가 직접 번쩍이며 진동하는 피격 연출 연동.
- 📐 **5:5 황금 분할 레이드 레이아웃 & 100dvh 제로 스크롤 (`BattleView.tsx`, `BattleFieldLanes.tsx`)**:
  - 보스방 입장 시 레인 상단의 낭비되던 빈 공간을 걷어내고, 하단 5개 레인(D0~D2 쫄몹)과 플레이어 아바타, 스킬바를 콤팩트하게 밀집.
  - **"상단에는 거대한 대악마가 내려다보고, 하단에서는 플레이어가 쫄들을 도륙하며 진격하는"** 완벽한 보스 레이드 비주얼 달성.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).

### 37. 와이드 768px (`max-w-3xl`) 대형 시네마틱 던전 승리 화면(Grand Victory Screen) 구축
- 🏆 **200px 대형 영웅 승리 아트워크 배너 (`h-44 sm:h-56`) & 768px 와이드 팝업 (`DungeonVictoryModal.tsx`)**:
  - 기존 576px의 좁고 찌그러져 있던 승리 팝업을 **768px 와이드 대형 규격**으로 시원하게 확장.
  - 대성당의 서광과 승리의 환희를 담은 **200px 고화질 대형 시네마틱 아트워크(`dungeon_victory_heroic.webp`)**를 시원하게 배치.
  - 대형 골드 타이틀 `[🏆 던전 완전 정복!]`, `[난이도/챕터 돌파 뱃지]` 및 우측 대형 `[📖 데커드 케인 감정 [F]]` 뱃지 결합.
- 💎 **대형 3대 보상 카드 & 3열 와이드 전리품 장비 쇼케이스**:
  - 💰 골드, 💎 샤드, ✨ EXP 보상 수치를 큼직한 폰트와 럭셔리 프레임 카드로 출력.
  - 획득 장비 목록을 **3열 와이드 그리드 (`grid-cols-3`)**로 쾌적하게 배치하여 추천 장착 및 전리품 확인 편의성 극대화.
  - 하단 48px 대형 액션 버튼 `[⚔️ 다음 진격 (Space)]` & `[🏰 마을 귀환 (Esc)]` 탑재.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).
