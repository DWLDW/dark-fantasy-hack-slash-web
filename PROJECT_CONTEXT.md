...
- 🔮 **룬워드 공방 소켓 베이스 아이템 최고 품질순 정렬 & 부위별 필터 (`TownView.tsx`)**:
  - **부위별 5대 필터 탭**: `[전체] [⚔️ 무기] [🥋 갑옷] [👑 투구] [🛡️ 방패]`와 각 부위별 보유 수량 뱃지 제공.
  - **다차원 복합 정렬 셀렉터**:
    - `💎 최고 품질순 (기본)`: 품질(유니크>세트>레어>매직>노멀) ➔ 소켓 수(6>1) ➔ 티어(Elite>Exceptional>Normal) ➔ 기본 스탯.
    - `🔢 소켓 수 많은 순` / `⭐ 티어 높은 순` / `⚔️ 공격/방어 스탯순`.
    - 아이템 카드에 등급 뱃지(`unique`, `set`, `rare`, `magic`, `normal`), 소켓 수, 공격력/방어력 수치 명확화.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).
