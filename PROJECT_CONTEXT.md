...
- 👹 **마스킹 없는 100% 선명한 거대 보스 전신/반신 시네마틱 아레나 (`BossHUD.tsx`)**:
  - 답답한 사각 액자 및 어두운 오버레이를 전면 철폐하고, 상단 140px~185px(화면 상단 약 40%) 전체를 꽉 채우는 **고해상도 보스(안다리엘/듀리엘/메피스토/디아블로/바알) 전신/반신 일러스트 시네마틱 스테이지** 완성.
  - 보스 머리 위 `[👑 대악마 타이틀]` + `[속성 뱃지]`, 발밑 와이드 **24px 2단 심연 체력 게이지**, **전멸기 BREAK 카운터**, **약점 레인 조준 버튼** 통합.
