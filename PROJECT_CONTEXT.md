...
- 👹 **5대 액트 최종 보스 대형 와이드 포트레이트 배너 HUD (`BossHUD.tsx`)**:
  - `boss_andariel.webp`, `boss_duriel.webp`, `boss_mephisto.webp`, `boss_diablo.webp`, `boss_baal.webp` 고화질 대형 와이드 포트레이트 연동.
  - 보스방 진입 시 상단 HUD 전체에 보스의 거대한 아트워크가 위압감 넘치게 배경으로 깔리고, 좌측 52px 고화질 포트레이트 프레임과 심연의 HP/그로기/차지 카운터가 결합되어 극강의 보스전 몰입감 선사.
- 🌐 **실서버 배포 완료**:
  - `https://193.122.127.129.sslip.io` (Oracle Cloud Ubuntu 24.04).
