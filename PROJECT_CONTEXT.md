...
- 🎒 **인벤토리 모달 혁신 (2터치 원스톱 장착 & 스크롤 제거) (`InventoryModal.tsx`, `inventory/`)**:
  ...
  - 하단 플로팅 퀵 액션 바: 아이템 선택 시 하단에 `[아이템명 + 스탯 변동치 + ⚔️ 원터치 장착 + 🔒/💰]` 팝업.
  - 상세 비교창 팝업 모달 (`isDetailModalOpen`) 분리로 필요한 순간에만 경량 팝업 열람.
  - `EquippedPaperdoll`: 슬롯 높이 58px 압축, 원터치 해제 버튼 강화.
  - `InventoryFilterBar`: `<Zap /> 추천장착`, `<Coins /> 일괄판매` 아이콘+키워드 초압축.
  - `ItemCompareTable`: 변동 스탯 중심 고밀도 인라인 칩 요약.
