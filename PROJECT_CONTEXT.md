...
### 9. 엑스트라 턴 컷인 마스킹 제거, 액트별 은은한 배경 아트워크, 보스 스킬 피해량 사전 표기 연동
- ⚡ **엑스트라 턴(1-More) 컷인 상단 재배치 & 중앙 마스킹 가림막 완전 제거 (`ExtraTurnCutin.tsx`)**:
  - 처형(Execute) 성공 시 데미지 롤러와 화면 중앙에서 겹치던 어두운 사다리꼴 가림막 박스를 완전히 없애고, 상단(`top-16`) 헤더 아래로 재배치하여 중앙 데미지 가시성을 100% 확보.
- 🖼️ **액트 1~5 고유 다크 판타지 은은한 배경 아트워크 탑재 (`public/images/acts/act1~5.webp`, `BattleFieldLanes.tsx`)**:
  - 액트별로 18~46KB의 초경량 WebP 배경 아트를 전장에 `opacity-25 mix-blend-luminosity`로 은은하게 블렌딩하여 전장 테마 몰입감 극대화.
