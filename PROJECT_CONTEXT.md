...
### 29. [1단계] 핵심 인게임 모달(인벤토리·스킬·스탯) 상용 게임급 UI/UX 전면 리뉴얼 완료
- 🛡️ **인벤토리 & 아이템 상세 1:1 Side-by-Side 비교 플로우 (`ItemDetailCard.tsx`, `InventoryModal.tsx`)**:
  - **대형 코어 주스탯 타이포그래피**: 무기 `⚔️ 45~68 물리 피해량`, 방어구 `🛡️ +125 기본 방어력`을 상단에 대형 텍스트로 시원하게 배치.
  - **세로 불릿 어픽스 리스트**: 18개 사각 박스 남발을 전면 제거하고, 상용 RPG 표준의 깔끔한 세로 불릿(`•`) 리스트로 전환.
  - **1:1 Side-by-Side Dual Card 비교 뷰**: 슬롯 선택 시 가방의 후보 장비와 현재 착용 장비가 좌우 나란히 배치되며, 각 스탯 우측에 `+15 ⚔️ (▲ 상승)` / `-5 🛡️ (▼ 하락)` 인라인 Diff 컬러 뱃지 표출.
  - **원터치 교체 액션 바**: 카드 바로 밑에 `[⚔️ 이 장비로 교체하기 (E)]` 및 `[장착 해제]` 배치.
- ⚡ **스킬북 & 원소 룬 3단 구획 및 보석 소켓 프레임 (`SkillRuneModal.tsx`)**:
  - **3대 테마 구역 분리**: 스킬 정보창(상단 다크골드/흑요석) / 레벨업 성장창(중단 에메랄드) / 원소 룬 각인창(하단 5대 보석 소켓 프레임)으로 시각적 뎁스 완벽 분리.
  - **보석 소켓 네온 글로우**: 화염(크림슨), 냉기(시안), 전격(옐로우), 독(에메랄드), 공허(퍼플) 각인 시 고유 글로우 테두리 연출.
  - **줄글 설명문 90% 제거**: 개발자용 안내문 삭제 및 단축키 지정 칩 일원화.
- 📊 **캐릭터 스탯창 원터치 컨트롤러 & 저항 게이지 (`CharacterModal.tsx`)**:
  - **글로벌 투자 단위 스위치**: 상단 `[+1P | +5P | +10P | MAX]` 칩 1개로 투자 단위 전환.
  - **단일 [+] 액션 버튼**: 18개 복잡한 버튼을 6개의 시원한 단일 `[+]` 버튼으로 정돈.
  - **4대 원소 저항 미니 게이지 바**: 75% 캡 충족도를 보여주는 직관적인 프로그레스 바 적용.
